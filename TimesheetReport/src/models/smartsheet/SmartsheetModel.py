...
class SmartSheets:
    def __init__(self, list_sheet=None, holiday=[], time_off={}, timedelta=1):
        ...
        self.info   = {}
        self.timedelta  = CommonUtils().get_prev_date_by_time_delta(timedelta)
...
class Sheet():
    def __init__(self, smartsheet_obj, sheet_name, latest_modified, sheet_id):
        ...
        self.latest_modified  = CommonUtils().convert_date_to_string(latest_modified)
        self.is_parse       = True
    ...
    def parse_sheet(self):
        ...
        modified_at = CommonUtils().convert_date_to_string(sheet.modified_at)
        if self.latest_modified == modified_at:
            CommonUtils().println('Skip parsing sheet: %s'%(self.name), 'info')
            self.is_parse       = False
        else:
            CommonUtils().println('Parsing sheet: %s'%(self.name), 'info')
    ...
    def parse_task(self):
        ...
        if self.start_date != None and self.end_date != None and self.assign_to != None:
            ...
            if self.end_date >= self.timedelta:
                self.list_date = CommonUtils().get_work_days(self.start_date, self.end_date, time_delta=self.timedelta)
